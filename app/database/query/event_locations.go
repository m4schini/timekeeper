...
func (q *Queries) GetLocationsOfEvent(eventId int) (ls []EventLocationModel, err error) {
	rows, err := q.DB.Query(`
SELECT l.id, l.name, l.file, l.osm_id, ehl.name as relationship, ehl.id as relationship_id
...
ORDER BY relationship`, eventId)
	...
	for rows.Next() {
		...
		err = rows.Scan(&l.ID, &l.Name, &l.File, &l.OsmId, &l.Relationship, &l.RelationshipId)
		if err != nil {
			return nil, err
		}
