...
func (c *Commands) CreateTimeslot(m model.CreateTimeslotModel) (id int, err error) {
	...
	row := c.DB.QueryRow(`
INSERT INTO timekeeper.timeslots (event, title, note, day, start, room, role, duration) 
VALUES ($1, $2, $3, $4, $5, $6, $7, ($8 * interval '1 second'))
RETURNING id`, m.Event, m.Title, m.Note, m.Day, m.Timeslot, m.Room, m.Role, int(m.Duration.Seconds()))
